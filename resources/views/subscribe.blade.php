...
if (str_contains($PlanName, 'joomla')) {
    ...
    $contract = '18k';
    $PlanName = 'magento';
} elseif (str_contains($PlanName, 'docker')) {
    $contract = '18k';
    $PlanName = 'Docker';
} elseif (str_contains($PlanName, 'linode')) {
    $contract = '18k';
    $PlanName = 'Linode';
} elseif (str_contains($PlanName, 'aws')) {
    $contract = '18k';
    $PlanName = 'AWS';
} elseif (str_contains($PlanName, 'digitalocean')) {
    $contract = '18k';
    $PlanName = 'digitalocean';
} elseif (str_contains($PlanName, 'react')) {
    $contract = '18k';
    $PlanName = 'React';
} elseif (str_contains($PlanName, 'react')) {
...
<head>
    ...
    <title>Subscribe: <?php echo $skill; ?> </title>
...
<body data-theme="light">
                ...
                <div class="row no-gutters align-items-center justify-content-center">
                    <div class="col-lg-6 col-md-9 col-xs-11 order-2 order-lg-1">
                        <div class="pt-9 pt-sm-13 pb-10 pb-md-15 dark-mode-texts pr-md-11 pr-lg-0">
                            <div class="mb-11">
                                @if (!Session::has('success') &&(!$milestone))
                                <h2 class="font-size-11 mb-8 pr-sm-10 pr-md-14 pr-lg-0 pr-xl-10" data-aos="fade-up" data-aos-duration="300" data-aos-once="true"><img class="fa" src="https://cdn.budgetphp.com/assets/brands-light/{{strtolower($PlanName)}}-brands.svg" style="width:55px;"> <?php echo $skill; ?></h2>

                                <p class="font-size-7 mb-0 pr-xl-15" data-aos="fade-up" data-aos-duration="500" data-aos-once="true">Use this form to pay <b><?php echo currency($amount, 'USD', currency()->getUserCurrency()); ?></b> every {{$interval_count}} <?php echo $interval . 's'; ?> in the {{currency()->getUserCurrency()}} currency. </p>
                                @endif
                                @if (!Session::has('success') &&($milestone))
                                <h2 class="font-size-11 mb-8 pr-sm-10 pr-md-14 pr-lg-0 pr-xl-10" data-aos="fade-up" data-aos-duration="300" data-aos-once="true"><img class="fa" src="https://cdn.budgetphp.com/assets/brands-light/{{strtolower($PlanName)}}-brands.svg" style="width:55px;"> <?php echo $skill; ?></h2>
                            ...
                            <div class="pr-lg-17 pr-xl-25" data-aos="fade-up" data-aos-duration="700" data-aos-once="true">
                                ...
                                <p class="mt-5">There are four ways to post tasks/issues with us.</p>
                                ...
                                @endif
                                <form role="form" action="{{env('APP_URL')}}/stripe-subscription" method="post" class="validation" data-cc-on-file="false" data-stripe-publishable-key="{{ env('STRIPE_KEY') }}" id="payment-form">
                                    @csrf
                                    ...
                                    <div class="form-group mb-7 card required">
                                        ...
                                        <input id="cardnum" value="" autocomplete='off' class='card-num form-control form-control-lg bg-white rounded-4 text-bali-gray pl-8 font-size-5' type='text' placeholder="Card Number">
                                    </div>
                                    ...
                                    <div class="max-w-435">
                                        ...
                                        <input type="hidden" name="interval_count" value="<?php echo $interval_count; ?>">

                                        <input type="hidden" name="milestone" value="<?php echo $milestone; ?>">
                                        ...
                                        <button class="btn btn-blue-3 w-100 h-55 rounded-4 " type="submit">Subscribe Now (<?php if ($milestone) {
                                                                                                                                $amountbtn = $amount / $milestone;
                                                                                                                            } else {
                                                                                                                                $amountbtn = $amount;
                                                                                                                            }
                                                                                                                            echo currency($amountbtn, 'USD', currency()->getUserCurrency());
                                                                                                                            echo ' ' . ucfirst($interval); ?>ly)</button>
                    ...
                    <div class="col-lg-6 col-md-9 col-xs-11 order-1 order-lg-2 d-none d-lg-block d-xl-block">
                        <div class="text-center pt-8 pt-lg-0" data-aos="fade-left" data-aos-duration="300" data-aos-once="true">

                            @if (!Session::has('success') &&($milestone))
                            <p class="font-size-7 text-white text-center" data-aos="fade-up" data-aos-duration="300" data-aos-once="true"> <?php echo $skill; ?> Milestone Payment Plan</p>
                                ...
                                <tbody>
                                    ...
                                    @while($x <= $milestone) <tr>
                                        ...
                                        <td><?php if ($x == $milestone) {
                                                echo '<span class="badge badge-light">Delivery</span>';
                                            } else {
                                                echo 'Build';
                                            } ?> </td>
                            ...
                            <p class="font-size-6 mb-0 text-left text-center text-white" data-aos="fade-up" data-aos-duration="500" data-aos-once="true">Use this payment form to automatically pay <b><?php $amountx = $amount / $milestone;
                                                                                                                                                                                                        echo currency($amountx, 'USD', currency()->getUserCurrency()); ?></b> {{currency()->getUserCurrency()}} each month with the last automatic milestone payment occuring on or about the {{ \Carbon\Carbon::now()->addMonths($milestone)->subMonths(1)->format('jS \of F Y')}}. </p>
                            @endif
                            @if (!$milestone)
    ...
    <div id='recaptcha' class="g-recaptcha" data-sitekey="{{env('RECAPTCHA_V2_INVISIBLE_SITE')}}" data-callback="setResponse" data-badge="bottomright" data-size="invisible" style="display:none !important;"></div>
    ...
    @if (str_contains(request()->route('skill'), 'maintenance'))
    @component('components.contracts.maintenance')
    @endcomponent
    @elseif (str_contains(request()->route('skill'), 'design'))
    @component('components.contracts.design')
    @endcomponent
    @elseif (str_contains(request()->route('skill'), 'development'))
